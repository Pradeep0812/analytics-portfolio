...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return {
        title: `${project.title} | Excel | Analytics Portfolio`,
...
export default function ExcelProjectPage({ params }: PageProps) {
    ...
    return (
        <div className="pt-20 lg:pt-24">
            <section className="py-12 lg:py-16 bg-gradient-to-br from-emerald-50 via-teal-50 to-white dark:from-surface-900 dark:via-surface-900 dark:to-surface-950">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    <nav className="flex items-center gap-2 text-sm text-surface-500 mb-6">
                        <Link href="/" className="hover:text-primary-600 transition-colors">Home</Link>
                        <span>/</span>
                        <Link href="/excel" className="hover:text-primary-600 transition-colors">Excel</Link>
                        <span>/</span>
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
                    ...
                    {project.powerbi_embed_url && (
                        <div className="flex items-center gap-4">
                            <a
                                href={project.powerbi_embed_url}
                                download
                                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-medium transition-colors shadow-lg hover:shadow-xl"
                            >
                    ...
                    <div className="pt-8 border-t border-surface-200 dark:border-surface-700">
                        <Link href="/excel" className="inline-flex items-center gap-2 text-primary-600 dark:text-primary-400 hover:underline font-medium transition-colors">
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16l-4-4m0 0l4-4m-4 4h18" />
                            </svg>
                            Back to Excel Projects
                        </Link>
                    </div>
                </div>
            </section>
