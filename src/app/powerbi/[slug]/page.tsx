...
export default function PowerBIProjectPage({ params }: PageProps) {
    const project = getProjectBySlug('powerbi', params.slug);
    ...
    return (
        <div className="pt-20 lg:pt-24">
            {/* Header */}
            <section className="py-12 lg:py-16 bg-gradient-to-br from-amber-50 via-orange-50 to-white dark:from-surface-900 dark:via-surface-900 dark:to-surface-950">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <nav className="flex items-center gap-2 text-sm text-surface-500 mb-6">
                        ...
                        <span>/</span>
                        <Link href="/powerbi" className="hover:text-primary-600 transition-colors">
                            Power BI
                        </Link>
                        <span>/</span>
                        <span className="text-surface-900 dark:text-white truncate max-w-[200px]">
                            {project.title}
                        </span>
                    </nav>
                    ...
                    <div className="flex flex-wrap gap-2 mt-6">
                        {project.tools.map((tool) => (
                            <span
                                key={tool}
                                className="px-3 py-1.5 rounded-lg bg-surface-100 dark:bg-surface-800 text-surface-700 dark:text-surface-200 text-sm font-medium"
                            >
                                {tool}
                            </span>
                        ))}
            ...
            <section className="py-12 lg:py-16">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
                    ...
                    {/* PDF Viewer */}
                    {project.pdf && (
                        <PDFViewer src={project.pdf} title={`${project.title} - Report`} />
                    )}
                        ...
                        <div className="pt-6 border-t border-surface-200 dark:border-surface-700">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {project.tags.map((tag) => (
                                    <span
                                        key={tag}
                                        className="px-3 py-1 rounded-full bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-300 text-sm"
                                    >
                                        {tag}
                                    </span>
                                ))}
                    ...
                    <div className="pt-8 border-t border-surface-200 dark:border-surface-700">
                        <Link
                            href="/powerbi"
                            className="inline-flex items-center gap-2 text-primary-600 dark:text-primary-400 hover:underline font-medium transition-colors"
                        >
                            <svg
                                className="w-5 h-5"
                                fill="none"
